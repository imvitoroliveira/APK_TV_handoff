    ...
    try {
      ...
      if (file.name.endsWith('.json')) {
        ...
        return;
            ...
            // Normalize spreadsheet headers to the compact fields used by the
      // client-login Edge Function. Supports accents and common aliases.
      ...
      data = data.map((row: any) => ({
        ...row,
        u: findColumn(row, ['u', 'usuario', 'user', 'login']) || String(row.u ?? '').trim(),
        p: findColumn(row, ['p', 'senha', 'senhas', 'password', 'pass']) || String(row.p ?? '').trim(),
        e: findColumn(row, ['e', 'expiracao', 'data de expiracao', 'vencimento', 'validade']) || String(row.e ?? '').trim(),
        t: findColumn(row, ['t', 'status', 'situacao', 'situacao do cliente']) || String(row.t ?? 'Ativo').trim(),
        '7': findColumn(row, ['7', 'proximo vencimento', 'a vencer', 'expirando']) || String(row['7'] ?? '').trim(),
      }));
