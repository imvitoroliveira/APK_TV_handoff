...
const GlobalPlayer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const buildSupabaseStreamProxy = (url: string) => (
      `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/stream-proxy?url=${encodeURIComponent(url)}&apikey=${encodeURIComponent(import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY)}&_cb=${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
      ...
      {(isFullscreen || isMini) && (
        ...
        >
          ...
          <video
            ...
            poster={framePosterRef.current || currentMedia?.poster}
            muted={isMuted}
