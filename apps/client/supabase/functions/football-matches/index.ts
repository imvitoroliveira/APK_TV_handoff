...
async function decidePolling(supabase: any, brDate: string): Promise<PollingDecision> {
  ...
  // Count live games (including halftime). Refresh at most once per minute;
  // the client itself polls every 5 seconds, so this avoids API overuse while
  // ensuring a halftime row cannot remain frozen indefinitely.
  const liveGames = games.filter((g: any) => g.status === "ao_vivo" || g.status === "intervalo");
  if (liveGames.length > 0) {
    const latestLiveUpdate = liveGames.reduce((latest: Date, g: any) => {
      const t = new Date(g.atualizado_em);
      return t > latest ? t : latest;
    }, new Date(0));
    const liveAgeSeconds = (now.getTime() - latestLiveUpdate.getTime()) / 1000;
    if (liveAgeSeconds >= 60) {
      return { shouldFetch: true, reason: `${liveGames.length} jogos ao vivo com dados há ${Math.round(liveAgeSeconds)}s`, hasLiveGames: true, gamesStartingSoon: 0 };
    }
    return { shouldFetch: false, reason: `Dados ao vivo atualizados há ${Math.round(liveAgeSeconds)}s`, hasLiveGames: true, gamesStartingSoon: 0 };
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // For client requests, return fresh-enough rows from DB. If the polling
    // decision says they are stale, continue into the scrape cascade.
    if (!isCronRequest) {
      const { data } = await supabase
        .from("jogos_ativos")
        .select("*")
      ...
      if (data && data.length > 0) {
        ...
        console.log(`[football] Client refresh: ${decision.reason}`);
      } else {
