...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'login') {
      ...
      // Normalize spreadsheet cells before comparing. This prevents a hidden
      // trailing space in a username/password cell from causing a false failure.
      const client = clients.find((c: any) =>
        String(c?.u ?? '').trim() === username &&
        String(c?.p ?? '').trim() === password
      );
      ...
      const status = String(client.t ?? '').trim().toLowerCase();
      const inactiveStatuses = new Set([
        'inativo', 'inativa', 'bloqueado', 'bloqueada',
